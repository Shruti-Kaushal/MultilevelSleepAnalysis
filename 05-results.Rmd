...
ggplot(mydata, aes(mydata$user, mydata$M)) + geom_point() + scale_x_continuous(breaks = scales::pretty_breaks(n = 17)) + labs(x = "User ID", y = "Time in Mintues", title = "Latency per User") + theme_bw()

ggplot(stack(dplyr::select(mydata, c('M'))), aes(x = ind, y = values))  + geom_boxplot() + geom_jitter() +labs(x = "", y = "Ratio Values", title = "Distribution of Sleep Ratio and Sleep Fragmentation Index in Cohort") 
...
```{r}
ggplot(mydata, aes(x = log(`M`, 10), y = log(`Efficiency`, 10))) + geom_point() + geom_smooth(method="lm") + labs(title = "Efficiency vs Sleep Quality (log10 scale)")
