...
## Sleep
The sleep file includes measurements ranging from the time the user gets into bed to the time that he gets out of bed. This file also includes measurements for total minutes spent in bed, total sleep time, number of times the user wakes up etc. 
...
![Sleep Measurements of User 1](./img/Sleep Measurements_User1.png)
...
## Saliva
`Saliva.csv` collected `Melatonin` and `Cortisol` concentrations in the saliva for each participants before going to bed and after waking up. However, no data was provided for User_21 due to problem in the salivary samples that were collected.

Saliva samples were collected twice - once before sleep and once after waking up. From each of the samples, the concentration of cortisol and melatonin were calculated. Melatonin levels are reported in μg per μg of protein while cortisol levels are reported in μg per 100 μg of protein. The data contains the normalized concentrations of both hormones and so no transformation or normalization is needed to use this data.
